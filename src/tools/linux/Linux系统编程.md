...
注意Linux下删除文件的机制：不断将st_nlink -1，直至减到0为止。文件没有目录项，并且没有任何进程在使用该文件，此时inode中维护的计数器会变成0，而且操作系统会把inode和指向的磁盘区域标记成空闲。但是inode里面的东西还是原先的东西，所以可以通过数据恢复工具找到删除的文件。该文件将会被操作系统择机释放。（也就是释放文件所占用的磁盘区域，允许其他数据覆盖该区域。）
