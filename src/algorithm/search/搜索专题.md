...
```ad-tip
collapse: open
01背包问题是一个很经典的问题，它有多种解法。DFS是其中一种，在学习动态规划的时候还会提到它。
```

> 第i件物品无非就是选和不选两种情况，在搜索的过程中`DFS`函数必须要记录当前处理的物品编号`index`，当前背包的容量`sumW`，当前的总价值`sumC`。

* 当不选第`index`个物品时，那么`sumW`,`sumC`是不变的，接着处理第`index+1`个物品，也就是`DFS(index+1, sumW, sumC)`。
* 当选择第`index`个物品时，`sumW`变成`sumW+w[index]`，`sumC`变成`sumC+v[index]`，接着处理第`index+1`个物品，也就是`DFS(index+1, sumW+w[index],sumC+v[index])`。边界条件也就是把最后一件物品也处理完了，即`index=n`（注意默认`index`从`0`开始）。
...
> 当然了01背包问题如果用`DFS`来解决的话，当数据比较大时，是不能`AC`的。这道题会超出时间限制（`TLE`）。即便经过剪枝也是无能为力的。
...
>**题目描述**
>把`1~n`这`n`个整数排列成一行后随机打乱顺序，输出所有可能的次序。
>**输入格式**
>一个整数`n`
>**输出格式**
>按照从小到大的顺序输出所有方案，每行`1`个。首先，同一行相邻两个数用一个空格隔开。其次，对于两个不同的行，对应下标的数一一比较，字典序较小的排在前面。
>**数据范围**
>`1 ≤ n ≤ 9`
...
>给定一个`n*m`大小的迷宫，其中`*`代表不可通过的墙壁，而`“ . ”`代表平地，`S`表示起点，`T`表示终点。移动过程中，如果当前位置是`(x,y)`(下标从`0`开始)，且每次只能前往上下左右`（x,y+1）、（x,y-1）、（x-1,y）、（x+1,y）`四个位置的平地，求从起点到达终点`T`的最少步数。
...
本题`BFS`或者`DFS`都可以实现，用`BFS`的话因为是板子题嘛直接照着模板敲就好了;

我们再来看一下如何用`DFS`来解决这个问题。用`DFS`搜索出所有可以从起点到终点的路径，并记录步数。DFS函数记录当前搜索的点的坐标`(x,y)`和所用步数`step`。用一个变量`minsize`更新最小步数`minsize = min(minsize, step)`。其余的操作其实和上一篇文章的迷宫问题求方案数是差不多的啦。

下面给出分别用`DFS`和`BFS`实现的代码：
~~~tabs 
---tab DFS代码
```cpp
#include <iostream>
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/1144f477e46f268b4caf1d253797ebc0.jpeg#pic_center)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/865b6ad1c923a610eaee015f85c7df8f.png#pic_center)
***
## 蓝桥杯真题（搜索算法）
### 1. 2017-试题D 方格分割

[题目链接](https://www.lanqiao.cn/problems/644/learning/)
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f7e006bf80800cb461e705c34d3e8d35.png#pic_center)
...
我们从中心点`(3,3)`出发，向上，下，左，右四个方向进行`DFS`搜索，当到达边界时说明一条分割方案就形成了。然后回溯继续寻找下一个分割方案。当然还有一些细节需要注意。

 - 当一个点`(x, y)`被搜索的时候，它的中心对称点`(6 - x, 6 - y)`就也就不能被搜索。因此需要标记两个点，即`maze[x][y] = 1`和`maze[6 - x][6 - y] = 1`。（表示该点已经访问过了）
 - 由于题目中说旋转对称属于同一种割法，那么一种分割方案绕着中心点顺时针旋转得到的方案和原来的方案是同一种。因此我们在`DFS`完之后的`ans`要除以`4`。
...
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/cd93e5e8feac13ac6ecf850bd8a4b40a.png)
***
### 2. 2019-试题A 组队

[题目链接](https://www.lanqiao.cn/problems/604/learning/)
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/cf143f6edb03f1506c0a15a06d0bdb4b.png#pic_center)
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/173e8c10532b3cc89b00cbe41e9f331b.png#pic_center)

**题目分析**

这道题是填空题，问评分之和最大可能是多少，由于该题的数据量不大，我们完全可以手算。

另外这道题是一道典型的`DFS`问题，对于一号位来说，它有20种选择，而对于二号位来说，它有除一号位选择的球员外的19种选择，以此类推。那么`DFS`的思路也就很明确了。我们从一号位开始`DFS`，维护当前正在选择的第`index`位，以及当前的评分之和`sum`。当`1~5`号位都`DFS`完，用`maxsum`来更新最大的评分之和。
...
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e7fa39661eb06a5b0e0d7587c7f26bd9.png)

**Answer 2. 观察+手算**
>很容易就可以发现评分之和最大是多少，这里就不详细展开说了。
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/9da1f38959342f8311705e2c978752a0.png#pic_center)
***

### 3. 2018-试题I 全球变暖

  [题目链接](https://www.lanqiao.cn/problems/178/learning/)
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/577729a7732b9ab56859804f792cbf4c.png#pic_center)
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/f39f60083d1dc7b5267a79530899f92c.png#pic_center)
...
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/02b77247a1782090e383fc006beee906.png#pic_center)
>当然这道题同样可以用BFS来实现，小伙伴们可以动手做一下，加深对DFS和BFS的理解。
***
### 4. 2019-试题E 迷宫

[题目链接](https://www.lanqiao.cn/problems/602/learning/)
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b95d885aaf8cf50e04458041fb7832a9.png#pic_center)
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/a9574a1ed2b24e0e2df098e4325c9952.png#pic_center)
**题目分析**

这道题和[蓝桥杯算法竞赛备考（一）—— 搜索专题（下）(C++)](https://blog.csdn.net/CCEVV/article/details/122998601?spm=1001.2014.3001.5501)中的迷宫最短路问题是类似的，只不过这道题还要记录一下搜索的路径，输出字典序最小的那条路径。
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/eee6746d93dd6b6cd9bf269dd321c25b.png#pic_center)
...
在搜索的过程中，记录到达该点的路径，我们可以设置个`dic`数组，代表`DLRU`。将某点入队时，`Node.ans = temp.ans + dic[i];`。
...
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/3a557668aa0b575c56726d3d0fcc6580.png#pic_center)
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/39df1e09e36f7230d565324ceec08fed.jpeg#pic_center)
***
